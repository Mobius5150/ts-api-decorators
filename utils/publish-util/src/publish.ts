...
const disabled = ['ts-api-decorators-azure-function'];
...
program
	...
	.option('--registry <registry>', 'The registry to publish to', 'https://registry.npmjs.org/')
	.action(async (version: string, opts: PublishCommandArgs) => {
		...
		if (opts.publish) {
			...
			console.warn(`🚨 Running in publish mode - I'll publish packages to ${opts.registry}`);
			opts.build = true;
		} else {
		...
		if (opts.publish) {
			...
			for (const pkg of packages) {
				...
				await runCommand('yarn', ['publish', '--no-git-tag-version', '--registry', opts.registry], pkg.path);
			}
...
function findPackages() {
	return fs
		.readdirSync(projectDirectories.packages, { withFileTypes: true })
		...
		function outputLog() {
			for (const log of logs) {
				if (log.stream === 'stdout') {
					process.stdout.write(chalk.gray(`\t${log.data}`));
				} else {
					process.stdout.write(chalk.red(`\t${log.data}`));
				}
			}
